...
class ModelArch(BaseModel):
    ...
    # --- Hybrid state-space geometry --------------------------------------
    # Nemotron-H, Granite 4 H, Falcon-H1, Qwen3-Next and friends replace most
    # attention layers with Mamba-2 mixers. The distinction is the whole point
    # of the architecture: a Mamba layer's per-sequence state is *constant*,
    # while an attention layer's KV grows with context. Nemotron-Nano-9B-v2
    # has 4 attention layers out of 56, so treating them all as attention
    # overstates its KV cache 14x.
...
class TrainingKnobs(BaseModel):
    ...
    #: How the packed-expert adapter path executes. ``"materialized"`` is the
    #: code as it runs today: PEFT's ParamWrapper builds the full effective
    #: weight (W + scale * B @ A) per targeted matrix as a graph tensor, which
    #: is saved for backward, holds the gathered ZeRO-3 leaf through the
    #: block's backward, and pays dispatch intermediates linear in context.
    #: ``"contracted"`` models the planned fix — the delta computed by
    #: contraction and the dispatch chunked — and drops those surcharges.
    #: Estimate with the default unless the improved kernels have landed.
