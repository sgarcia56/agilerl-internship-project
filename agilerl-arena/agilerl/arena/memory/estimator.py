...
def _engine_reservation_bytes(
    ...
    ``max_num_seqs`` is the *engine's* slot count, which the training knobs do
...
def estimate_training(
    ...
    # peak at one measured corner. On every path that forwards under
    # ``_amp_ctx`` the framework disables the casts for the *original*
    # forward (``_lora_input_cast_ctx`` in ``algorithms/core/base.py``), but
    # the checkpoint recompute runs after the context has restored the flag —
    # so one block's casts reappear during backward, for the rows of the one
    # graph being recomputed (``grad_rows``, not ``graph_rows``), and none
    # stand at the loss instant. The DPO sweep pins both halves: its
    # s4096/mb8 corners sit within 1-3% with one graph's recompute casts and
    # read -14% without them, +18% with both graphs'. SFT's loss path runs
    # outside ``_amp_ctx``, so it keeps its forward casts — see
    ...
    # never a (B, S, V) slab. Two fp32 tiles are live at the loss instant —
    # ``FusedLinearLogProbsFunction.backward`` recomputes the chunk's logits
    # in fp32 and ``torch.autograd.grad`` materialises that tile's gradient —
    # alongside the fp32 lm_head copy the chunk loop hoists
    # (``fp32_lm_head_operands``, both in ``algorithms/core/llm_ops/
    # fused_logprobs.py``). Measured twice at the bound loss instant:
    # the SFT timeline's peaks decompose as 519 + n x 256 MiB on
    # Qwen2.5-0.5B (151936 x 896 head), and the Gemma-4-E2B timeline reads
    # 2,069 MiB against 2,048 predicted (1,536 head + 2 x 256 tiles).
...
def estimate_generation(
    ...
    if arch.multimodal_tower_params or arch.per_layer_input_dim:
        # Measured on Gemma 4 E2B under the production ray-worker image: the
        # engine's device residency did not track gpu_memory_utilization at
        # all — flat at weights + ~3.9 GiB across budgets of 0.28-0.44,
        # sitting ~2.3 GiB *above* the budget at the low end and ~4 GiB below
        # at the high end — while the same engine under a different vLLM
        # filled whatever budget it was given. Engine construction is worse:
        # its transient profiling peak reached 1.76x the serving bar at 16
        # slots x 4096 context. Until an engine-side trace attributes the
        # multimodal profiling allocations, this bar cannot be trusted to
        # follow the budget knob on a multimodal checkpoint.
        warnings.append(
    ...
    # Under ZeRO this device holds only its rank's shard.
    #
    # With ``use_memory_efficient_params=False`` nothing is offloaded at all —
    # the trainer's base copy and its adapters sit on the device through the
    # rollout. That is how PPO currently runs (its value head is not restored
    # before the Triton forward), and it is what the PPO sweep's generation
    # peaks show: 0.7-1.1 GiB above the engine-only bar on a 0.5B model,
    # growing with rank — the trainer base plus its resident adapters.
...
def estimate_run(config: RunConfig) -> RunEstimate:
    ...
    # the quantization knob, not through a prequantized weight variant. Only
