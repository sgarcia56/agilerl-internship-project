...
#:
#: This does not move the generation total: the pool is subtracted from the
#: budget when sizing the KV cache and added back as residency, so it
#: cancels. It only decides how the bar is attributed. The pool scales with
#: captured batch sizes (``cudagraph_capture_sizes``, tracking
#: ``max_num_seqs``), not with context. vLLM 0.26 reports 40-93 MiB on the
#: models measured; 256 MiB is a round ceiling on that.
...
#: library workspaces (cuBLAS/NCCL/flashinfer handles) plus engine structures.
#: Present whether the engine is asleep or awake.
#:
#: Not a constant, and not only a property of the model. The sleeping-engine
#: floor rises with depth and with sequence slots: sleep level 1 hands back
#: weights and the KV pool but not the per-sequence scheduler state sized at
#: construction.
...
#: Every other term here is an allocation size; the device is charged the
#: reservation, which is larger because the allocator rounds to segment
#: boundaries and cannot always reuse a freed block for a differently-shaped
#: request. Applies to the whole torch-side subtotal. Training only:
#: generation runs inside vLLM's CuMem pool, reserved up front at
#: ``gpu_memory_utilization``.
#:
#: 0.07: the median of ``torch_max_reserved / torch_max_allocated`` over all
#: 184 stored training points (1.069 median, 1.077 mean, per-fixture medians
#: 1.020-1.189). Torch's own counters, so this is measured rather than
#: fitted — which it had not been before. The earlier 0.02 and 0.05 were
#: both chosen as whatever the drift bands would tolerate, because an
#: over-charged fp32 LoRA cast term (see
#: :func:`lora_input_cast_bytes`) was cancelling most of the real
#: reservation at exactly the backward-bound corners where it binds. With
#: the cast term corrected against its allocator traces, the measured
#: reserve fits: pooled training goes 4.53% -> 4.06% mean and 18.0% -> 13.1%
#: worst, and the pooled signed error lands at +0.9% — the over-prediction
#: side, which is where a gate belongs.
...
def kbit_upcast_bytes(
    ...
    upcast lands on the biggest unquantized block in the model. A profiled
...
def weight_bytes(
    ...
    tied model ``lm_head`` is the embedding table — so the upcast lands on a
...
#: How many state blocks vLLM keeps per sequence, by ``mamba_cache_mode``
#: (``vllm/v1/kv_cache_interface.py::MambaSpec.max_memory_usage_bytes``).
#:
#: ``none`` is the classic recurrent cache: one state per running sequence,
#: which is the whole appeal of an SSM. Prefix caching needs states at block
#: boundaries to resume from, and ``all`` keeps one per block --
#: ``ceil(max_model_len / block_size)`` of them, which reintroduces exactly the
#: context-length scaling the architecture exists to avoid. ``align`` is the
#: cheap middle: cache only the last token of each scheduler step when it lands
#: on a block boundary, which bounds residency at two.
...
#: Blocks' worth of expert gather resident at once, over and above the one
#: block :func:`moe_dispatch_bytes` already charges. Gradient checkpointing
#: recomputes a block at a time, so the dense path frees each block's
#: intermediates at its boundary -- the gathered expert copies are not freed
#: there. Measured against three MoEs (OLMoE 64 experts, granite-3.1-3b-a800m
#: 40, granite-3.1-1b-a400m 32) the estimator under-predicted training by 8.3%,
#: 12.3% and 14.3% of peak, always low, and the shortfall is proportional to
#: the gather. Fitted through the origin over 29 points, leave-one-model-out
#: held-out error falls from 9.69% to 4.01%.
#:
#: The *shape* is not identified: all three route top_k=8, so
#: ``tokens x top_k x hidden`` and ``tokens x hidden`` fit identically (R^2
#: 0.54 either way). Separating them needs an MoE with a different top_k.
#:
#: 4, not the 5.8 the regression produced. Scored against the stored corpus and
#: the two held-out MoEs together, 4 is the largest value that costs the corpus
#: nothing at all:
#:
#:   blocks   corpus mean   corpus worst   held-out MoE mean   worst
#:        0         3.64%         11.86%              13.31%  22.81%
#:        4         3.42%         11.86%               7.26%  12.86%
#:        5         3.43%         12.32%               5.71%  11.68%
#:        6         3.46%         16.19%               4.77%  10.51%
#:
#: Past 4 the OLMoE corners start over-predicting. Over-prediction is the safe
#: direction -- it blocks a run that would have fitted rather than OOMing one
#: that would not -- but there is no evidence for paying it, so the conservative
#: value stands until a fourth MoE says otherwise.
MOE_GATHER_RESIDENT_BLOCKS = 4


#: bitsandbytes' backward workspace, per gradient token. nf4 buys weight bytes
#: and costs activation bytes, and this is the cost: measured from paired
#: bf16/nf4 runs at four context lengths on four models, the delta erodes
#: linearly in gradient tokens at 68, 104, 160 and 194 KiB/token on
#: Qwen2.5-0.5B/1.5B/3B and Gemma 4 E4B. That tracks depth x width:
#:
#:   slope = 40 KiB/token + 1.5 bytes per (n_layers x hidden) per token
#:
#: R^2 0.982 over the four, and leave-one-model-out holds it to 12.9 KiB/token
#: mean held-out error -- 10% of the mean slope, against 100% for having no
#: term at all. Two parameters over four models is thin; treat the shape as
#: established and the constants as provisional.
...
def block_recompute_bytes(
    ...
    if arch.is_moe:
        ...
        # Active FFN width per token: the routed experts it lands on plus the
...
#: Non-torch device memory the *trainer process itself* holds beyond the CUDA
#: context: cuBLAS/cuDNN workspaces, kernel modules, attention autotuning.
#: Measured on the first SFT sweep (Qwen2.5-0.5B, A100, 21 points): NVML peak
#: minus torch reserved came out at exactly 1084 MiB on every point — context
#: (501 on that device) plus 583 MiB of library state, constant across
#: micro-batch, sequence length and rank. Charged only when the run starts no
#: vLLM engine: for colocated runs the engine-floor constants were read off
#: measured floors that already include the trainer's libraries, so charging
#: it there would double-count.
TRAINER_LIB_OVERHEAD_BYTES = 583 * MiB


#: ZeRO-3's backward surcharge on a packed-MoE hybrid, per token of every
#: completion row the rank holds — not per gradient token. Measured on
#: Nemotron 3.5 Lightning 30B-A3B (A100-80): worlds 4 and 2 anchor the fit
#: (4 and 8 rows per rank — the two row counts are what identify the row
#: scaling), world 3 is held out and scores within +1.6/-2.2 GiB. The
#: carrier is per-row state that survives each micro-step of the
#: accumulation window under ZeRO-3 — 0.15 MiB per row-token is the scale
#: of a row's checkpoint-boundary hidden states across half the stack —
#: alongside the gathered leaf's backward: re-gathered weights, the
#: effective-weight copies PEFT ``target_parameters`` materializes, dispatch
#: intermediates, and the gather churn's fragmentation
#: (reserved-over-allocated runs 14% here against ZeRO-2's 4%). One model,
#: one device: provisional, like the bnb workspace constants were on entry.
