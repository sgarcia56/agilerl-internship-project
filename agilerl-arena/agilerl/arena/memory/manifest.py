...
- the training manifest — the single source of truth for every knob,
...
This module turns those into a :class:`RunConfig`. Every knob below is read
...
def training_knobs_from_manifest(
    ...
    """The trainer-side knobs a validated manifest pins down.
...
def generation_knobs_from_manifest(
    ...
    """The engine-side knobs the manifest's (resolved) vLLM config pins down.
